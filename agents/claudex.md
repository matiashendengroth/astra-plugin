...
2. Write the task text verbatim to a scratch file, then run ONE of these with Bash, timeout 600000:
   ...
   Follow-ups reuse the previous Codex session instead of starting cold (far cheaper): a re-review after fixes is `~/.local/bin/claudex review -v -C "<dir>" --uncommitted --json --resume "<focus>"` (sends only the current diff and the previous findings); a build follow-up (test failure, reviewer finding) is `cat followup.md | ~/.local/bin/claudex build -v -C "<dir>" --resume`. Use these whenever the task says "re-review", "follow-up", "fix the findings", or names a previous build/review in the same directory.
...
4. Your final message must be exactly: the `RESULT:` line with that `.last.md` path, the `LOG:` line with the `.log` path, then the full contents of the `.last.md` file unchanged under a `--- CLAUDEX OUTPUT ---` line. Do not summarise, soften, or comment. If the wrapper failed or timed out, return its stderr instead, prefixed `FAILED:`. If it exited 5 (Codex usage limit), return its message prefixed `LIMITED:` and do not retry.
